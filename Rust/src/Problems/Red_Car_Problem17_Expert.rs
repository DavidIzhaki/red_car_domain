...
fn main() {
    ...
    let pink_car = Vehicle::new(VehicleKind::VerticalCar, [0, 1], "pink_car".to_string());
    grid.place_object(&pink_car).unwrap();
    grid.add_vehicle(pink_car);

    let purple_car = Vehicle::new(VehicleKind::VerticalCar, [1, 1], "purple_car".to_string());
    ...
    let brown_car = Vehicle::new(VehicleKind::HorizontalCar, [3, 5], "brown_car".to_string());
    grid.place_object(&brown_car).unwrap();
    grid.add_vehicle(brown_car);

    let white_gray_car = Vehicle::new(VehicleKind::HorizontalCar, [1, 3], "white_gray_car".to_string());
    grid.place_object(&white_gray_car).unwrap();
    grid.add_vehicle(white_gray_car);

    let yellow_truck = Vehicle::new(VehicleKind::VerticalTruck, [4, 2], "yellow_truck".to_string());
