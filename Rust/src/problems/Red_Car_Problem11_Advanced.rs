...
fn main() {
    ...
    let orange_car = Vehicle::new(VehicleKind::VerticalCar, [4, 0], "orange_car".to_string());
    grid.place_object(&orange_car).unwrap();
    grid.add_vehicle(orange_car);

    let blue_sky_car = Vehicle::new(VehicleKind::VerticalCar, [5, 0], "blue_sky_car".to_string());
    ...
    let green_over_car = Vehicle::new(VehicleKind::HorizontalCar, [0, 4], "green_over_car".to_string());
    grid.place_object(&green_over_car).unwrap();
    grid.add_vehicle(green_over_car);

    let purple_car = Vehicle::new(VehicleKind::HorizontalCar, [3, 3], "purple_car".to_string());
    grid.place_object(&purple_car).unwrap();
    grid.add_vehicle(purple_car);

    let white_gray_car = Vehicle::new(VehicleKind::HorizontalCar, [0, 5], "white_gray_car".to_string());
    ...
    let blue_truck = Vehicle::new(VehicleKind::VerticalTruck, [2, 3], "blue_truck".to_string());
    grid.place_object(&blue_truck).unwrap();
    grid.add_vehicle(blue_truck);

    let yellow_truck = Vehicle::new(VehicleKind::VerticalTruck, [3, 0], "yellow_truck".to_string());
